...
>
> 🔴 **第二十轮(最新)：抖动这条线暂停，等用户定方向。**
> 第十九轮的 transform 垫付**机制跑通了**(定一次→归零、无振荡、无残留、不卡死)，
> 但用户验收「**一点没变**」→ 已撤销(commit `e06960452`)，代码回到只有探针的状态。
> 📌 **最贵的教训见 L138**：我一共指认过四个病根，最后一个**实测存在、频率完全吻合**，
> 修掉之后观感零变化 —— 「每次都发生」是**必要条件不是充分条件**，
> 一次翻页里每次都发生的东西多得很，频率筛只能缩到"一类"，
> **从类到个只能靠"改掉它→症状变"，而那一步只有人眼能做。**
> ⚠️ **下一轮的第一件事必须是"小步 + 马上让用户看一眼"，别再攒大假设。**
> 剩下三个选项(收尾 / 查翻页动画本身 / 降级掉动画)列在 `NOTES-todo.md`「第二十轮」，**由用户定**。
>
> ~~第十九轮：首选方案已落地，等真机验收。~~（已撤销，下面留作背景）
