...
**先读**：本目录 STATUS.md 顶部各小节（2026-09-24/25 的 Reader、文章列表、首页修复等，均已提交并推送）、DECISIONS.md 的 ADR-016～027、LESSONS.md 第 30～41 条。根目录的 NOTES-progress.md / CURRENT-STATE.md 是 1.x 时期的旧记录，不代表当前状态。

**当前：文章列表顶部大图与缩略图（ADR-027，三步）——第 1 步已验收并推送（`636696cbb`）；第 2 步顶部大图（样式 E 整幅清晰铺满）已验收并推送；第 3 步滚动收缩进行中**
- 第 1 步：接入层从正文取首图、缩略图缩小解码与缓存；按用户给的 Reeder 截图重排文章行（来源图标/来源名/时刻贴右/17pt 标题与 1 行摘要/缩略图在时间下方）并按天分段、段标题吸顶。图标对齐标题字中线、摘要改用正文开头（上游 truncatedSummary）。全量 124/124（独立 DerivedData）。详见 STATUS 顶部。
- **本机测试必须用独立 `-derivedDataPath`**（LESSONS 41），并核对 `Ld` 步骤与测试总数。
- 技术待办：`testBarsHideAfterPinnedDownwardTravelAndReturnOnUpwardTravel`（阅读页栏显隐）偶发失败——带本次改动 5 次 4 过、已提交版本 5 次全过；只用阅读页，推测是排版回报与测试里同步滚动的时序竞争，未查证。
- 第 2 步（下一步）：顶部大图静态样子——A「标题印在纸上」、只放返回、有大图不放圆形小图标、无图退路、紧凑栏保留小图标 + 名字。用 Figma 节点 22:37 / 244:563 / 247:751，但标题处理按 ADR-027 不照稿。
- 第 3 步：滚动跟手收缩（MOTION-CONTRACT §11）。
- 上一项（长图三项改善 + 超长文章，ADR-026）已验收并推送 `cac72058c`。
