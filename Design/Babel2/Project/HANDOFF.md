...
- 动效规则在 `iOS/Babel2/Babel2Motion.swift`；占位条 `Babel2SkeletonView.swift`；同步箭头 `Babel2SyncSpinner.swift`。全量 151/151、UI Driver 1/1。详见 STATUS 顶部。
- 三批合并为一个提交（不含 pbxproj 签名 diff 与 `Shared/Localizable.xcstrings` stale 行）。

**第一批：全 App 字号收小一档 + 底栏等距（ADR-033）**
- 数字都在 `iOS/Babel2/Babel2Type.swift`；设置页不动。全量 145/145、UI Driver 1/1。详见 STATUS 顶部。

**四项反馈修正（ADR-032，已随上面一起提交）**
- 大图按钮毛玻璃圆底 + 状态栏渐变；「跟随系统」无后缀；全 app 弹出菜单统一毛玻璃（Babel2GlassMenu）；切回前台图标不再重载（接入层图标备份）。全量 144/144、UI Driver 1/1。详见 STATUS 顶部。

**最近完成：文章列表大图的刷新与更多（ADR-031，2026-09-25 用户真机验收通过，已推送）**
- 刷新（所有账户，点按后同步结束重载列表）与更多菜单（主页 / 拷贝地址 / 阅读模式 / 通知 / 重命名 / 取消订阅）。全量 142/142。详见 STATUS 顶部。

**最近完成：添加订阅页（ADR-030，2026-09-25 用户真机验收通过，已推送）**
- 「+」进入；单一搜索框（网址直连 / 关键词四类并行）、订阅到选文件夹、试读、订阅 / 确认后取消订阅；修复首页不随订阅源增删刷新。全量 140/140。详见 STATUS 顶部。
- 下一任务待用户指定。候选：设置里暂未放的项目（文章主题、分组方式等，需先做功能）；阅读页栏显隐测试偶发失败；阅读页上拉翻篇手势。

**最近完成：文章列表搜索（ADR-029，2026-09-25 用户真机验收通过，已推送）**
- 窄栏右上放大镜原地搜索（该源全部文章，全文 + 中文包含匹配），取消恢复；修复放大镜变形、搜索时滑动被补完拉回（LESSONS 44）、× 常驻。全量 137/137。详见 STATUS 顶部。

**最近完成：设置页（Slice 6，ADR-028，2026-09-25 用户真机验收通过，已推送）**
- 首页左上齿轮进入；8 类别与各编辑页、弹出单选菜单；接通配色模式、排序、已读确认、打开链接；修复右滑返回当前页变暗闪烁（LESSONS 43）。全量 136/136。详见 STATUS 顶部。

**最近完成：文章列表顶部大图与缩略图（ADR-027 三步全部完成，2026-09-25 用户真机验收通过，已推送）**
- 第 1 步缩略图 + Reeder 式文章行 + 按天分组（`636696cbb`）；第 2 步顶部大图样式 E（`dad42a5ee`）；第 3 步滚动跟手收缩 + 一行标题与图标错位修复（LESSONS 42）。详见 STATUS 顶部。
- **本机测试必须用独立 `-derivedDataPath`**（LESSONS 41），并核对 `Ld` 步骤与测试总数。
- 技术待办：`testBarsHideAfterPinnedDownwardTravelAndReturnOnUpwardTravel`（阅读页栏显隐）偶发失败，推测为排版回报与测试同步滚动的时序竞争，未查证。
- 下一任务待用户指定。

**之后的候选**：Slice 3 其余（列表搜索；大图上的刷新/搜索/更多按钮待功能做出后再加）、Slice 6（设置页：API key/模型、主题色等）、Reader 翻页手势（ADR-016 暂缓）。
...
10. **2026-09-24 更晚**：Reader 第 1 步已推送（`4c58dfcaa`）。第 2 步「滑动收缩」（方案 A）用户真机验收通过并已本地提交（未推送），同一提交内修正了既有时序敏感测试的等待方式。下一任务：Slice 4 第 3 步（固定后 12pt 方向累积、顶/底栏 180ms 线性显隐、底部工具栏占位），动手前先交书面方案。

11. **2026-09-24 最晚**：第 2 步已推送（`a1ae1a246`）。第 3 步「底栏与上下滑显隐」用户真机验收通过并已本地提交（未推送），Slice 4 三步完成。候选下一任务（待用户选择）：①修阅读页改已读/星标后列表与首页不即时刷新（改 Babel2LiveDataAdapters 的 articleCache 失效）；②Slice 3 Timeline hero；③Slice 5 Reader 高阶操作（下一篇、翻译、阅读模式、内置浏览器、长图位置决定）。

12. **2026-09-24 收尾**：Slice 4 已推送（`35d44c4a0`）。随后修「文章列表不随状态刷新」（原因分析已更正：首页本已刷新，缓存无关），2026-09-25 用户真机验收通过并已提交。下一任务：已读图标反转 + 打开文章自动标已读（用户已决定），动手前说明改动范围。

13. **2026-09-25**：列表原地刷新已提交（`2897962e9`，未推送）。已读图标反转 + 打开即标已读用户真机验收通过，已提交并与列表刷新一起推送。下一任务待用户选择：Slice 5（阅读页高级操作，需先定长图位置）或 Slice 3（Timeline hero）。

14. **2026-09-25 较晚**：Slice 5 按 ADR-016 分步。第 1 步「翻译」用户真机验收通过，已提交并推送。下一步：插入「阅读页对齐 Figma」（Figma 连接已接上；节点 22:38 / 117:263 / 117:317 / 143:444 / 143:73 / Reader Toolbar 21:5），之后再做阅读模式。

15. **2026-09-25 最新**：翻译已推送（`b702c0f01`）。「阅读页对齐 Figma」用户真机验收通过，已提交并推送。下一任务：Slice 5 第 2 步「阅读模式」（加入 ••• 菜单；底栏第 4 格按 ADR-016 改为长图占位），动手前先交书面方案。今后新增阅读页界面元素一律先用 Figma 连接读取设计稿（LESSONS 35）。

16. **2026-09-25 收尾**：Figma 对齐已推送（`29c47ee56`）。Slice 5 第 2 步「阅读模式」用户真机验收通过，已提交并推送。用户反馈入口放 ••• 操作成本高，待用户在「底栏第 4 格还给阅读模式 / 按订阅源总是阅读模式」等方案中选择，再继续第 3 步内置浏览器。

17. **2026-09-25 更晚**：阅读模式已推送（`eb0e180c6`）。入口调整（ADR-020）用户真机验收通过，已提交并推送。下一任务：Slice 5 第 3 步「内置浏览器 + 正文右缘左滑跟手进入」，先读 Figma 与 MOTION-CONTRACT §7 再交书面方案；产品合同中“长图入口只在底栏”一句需按 ADR-020 修订。

18. **2026-09-25 更晚**：阅读模式入口调整已推送（`8d0d75683`）。Slice 5 第 3 步「内置浏览器」用户真机验收通过，已提交并推送。下一任务：第 4 步「下一篇」（底栏按钮原地换文、不 push 新阅读页，ADR-016），动手前先交书面方案。

19. **2026-09-25 最晚**：内置浏览器已推送（`14cafca05`）。Slice 5 第 4 步「下一篇」用户真机验收通过，已提交并推送。插入：「文章列表底栏」（Figma Feed Toolbar 22:18/22:35，从 Slice 3 提前），之后回到 Slice 5 第 5 步生成长图。

20. **2026-09-25 收尾**：下一篇已推送（`eb076c0e0`）。「文章列表底栏」用户真机验收通过，已提交并推送。下一小步：首页底栏改用 `Babel2ScopeFilterControl` 修复切回「未读」时胶囊错位（用户选 A，需首页回归验收）；之后列表标题翻译开关，再回 Slice 5 第 5 步生成长图。
...
交接边界：这三行的证据全部停在"直接调用 `Babel2NavigationRestoration`/`Babel2SceneComposition.makeRoot(restoration:)` 这两个生产函数"，没有一条经过真实 `SceneDelegate.scene(_:willConnectTo:options:)` + 真实 `UISceneSession.stateRestorationActivity`——这是 `UISceneSession` 无法在测试里构造、且 `simctl` 无法脚本化复现"系统真的回收又恢复 scene"这个场景导致的结构性缺口，记在 [LESSONS.md](LESSONS.md) 第 24 条。要关闭这三行的最后一环，要么真机上真实发生一次（不可控），要么给生产代码加一个可测试的 restoration seam（这是代码改动，需要先问用户再做，本轮没做）。完整命令、九种输入矩阵和三份证据文件见 [VALIDATION.md](VALIDATION.md) 与 [PHASE1A-ACCEPTANCE.md](PHASE1A-ACCEPTANCE.md) 对应新增小节；本轮改动已提交 `733275b72`，未推送。

## Phase 1A：A11/A15 接手记录（2026-09-05，Asia/Tokyo，尚未提交）

同日第四批，延续 A4/A5/A9（已提交 `733275b72`，未推送）继续做 A11（30-cycle）与 A15（single owner + Gate A）。A11 除了已有和本轮新增的两个进程内 30 次单元测试，**额外做了一次 30 次真实冷启动**（每轮真正 uninstall/install/launch，不是进程内构造对象）——这是本轮 Phase 1A 证据里唯一一次做到"30 次全是独立真实 App 进程"的证据，结果 30 个不同 PID/session、30/30 valid+complete+零 legacy。A15 在整理 single-owner 交叉引用时，**顺手独立复核了已有的 Gate A 自动化测试本身**，发现它的扫描目录范围有缺口（漏了 `iOS/Babel2Integration/` 和 `iOS/Babel2ExternalActionParser.swift`），已经修复并加了一条精确的文件级白名单（`Babel2LiveDataAdapters.swift` 里合理但此前无监控的 `AccountManager.shared` 使用）。

交接边界：本轮改了两处测试文件——`Tests/NetNewsWire-iOSTests/Babel2FeatureGateTests.swift`（新增 A11 的进程内测试）和 `Tests/NetNewsWire-iOSTests/Babel2BoundaryTests.swift`（扩大 A15 的 Gate A 扫描范围），没有改任何生产代码。全量 Debug iOS test suite 重新跑过，`result: Passed`、`failedTests: 0`、`passedTests: 65`。仍未关闭：A15 的 persisted identity 边界（`LegacyIdentityCompatibility` 零实现，如实记录现状而非新回归）与完整 target/resource allowlist；物理 iPhone 上的真实 30-cycle。至此 Phase 1A 矩阵里我能靠命令行/模拟器自证的行（A0/A1/A2/A3/A6/A7/A11/A13/A14 完整通过；A4/A5/A8/A9/A10/A15 部分通过）已经做完；剩下的缺口全部需要真机或者一次需要用户先点头的代码改动（restoration 测试 seam）。完整内容见 [VALIDATION.md](VALIDATION.md) 与 [PHASE1A-ACCEPTANCE.md](PHASE1A-ACCEPTANCE.md) 对应新增小节。

## Phase 1A：A12 接手记录（2026-09-05，Asia/Tokyo，尚未提交）
