...
## ADR-023：文章列表底栏（Figma Feed Toolbar，从 Slice 3 提前；2026-09-25，用户“都按建议来”）

- 按 Figma「Feed Toolbar」(22:18 / 22:35)：全部标为已读（x=32，设计稿图标）/ 星标·未读·全部（x=104/201/290.5）/ 标题译开关（x=362，本步为灰色占位，下一小步接通标题批量翻译）。
...
## ADR-025：生成长图（Slice 5 第 5 步；2026-09-25，用户“按建议来”）

- 入口 ••• 菜单「生成长图」；原样复用旧版 `ArticleLongImageExporter` 与 `nnw_snapshot.js`（加载全部图片 → 整页导出 PDF → 拼长图，超长整体降分辨率不截断 → 页脚「分享自 <品牌名>」），所见即所得（译文/阅读模式/深浅色）。
- 新阅读页的日期/标题/署名是原生控件、不在网页里：生成前在网页顶部临时插入同样式标题区（纯文本写入），同一瞬间 `scrollBy` 同高度使屏幕内容不动，生成后移除并滚回。
- 过程提示：署名下方状态字「正在生成长图…」，失败显示原因（3 秒后消失）；不用系统转圈。完成后直接弹系统分享面板（含「存储图像」）。
- 否决：用定格截图盖住正文区来遮挡跳动——被完全盖住的网页系统不再绘制，导出一直等下去（实测卡死 8 分钟以上）。

## ADR-026：长图签名移到顶部、换新图标、存储成功提示（2026-09-25，用户“同意，提示选 A”；同日真机验收通过）

- 签名位置：「分享自 <品牌名>」签名条（76pt）画在长图最上方，细线在其下边隔开正文；不再有末尾页脚。
- 图标：新增普通图片资源 `Babel2ShareSignatureIcon`（AppIcon 浅/深版缩到 240px；iOS 运行时无法从 App 图标集取图）；单色版不用（只用于主屏着色模式）。
- 导出器改动（ADR-025 曾承诺零改动，本次经用户同意）：`ArticleLongImageExporter.export` 增加可选参数 `Signature(atTop:icon:)`，默认值 = 旧行为（末尾 + `ShareFooterIcon`），1.x 两处阅读页不传参、行为不变。
- 存储提示：分享面板完成回调中，仅 `activityType == .saveToCameraRoll`、completed 且无错误时，底栏上方正中浮出反色小胶囊「已存储到相册」，约 2 秒淡出，不接收点按，并发出旁白播报。Figma 无此设计稿，样式为近似。
- 否决：B「沿用署名下方状态字」——署名区随正文滚动，读到中途时看不到。
- 追加（2026-09-25，用户“同意改导出器，超长文章选 A”）：修复多页 PDF 拼图互相覆盖（每页限定在自己格子里，旧 `export` 同样修复）；新增 `exportImages`，超长文章拆成多张，每张 ≤ 25,000 像素、保持 2x，最多 10 张后才整体降分辨率；切点在目标位置往上 400 点内找一整行同色的行（行间空白），找不到才硬切；签名只在第 1 张顶部；分享面板一次交给全部图片。否决：B 放宽单张高度上限（约 6.4 万像素，可能被相册/微信压缩或拒收，无法在模拟器验证）；C 只修空白（156 像素宽几乎不可读）。

## ADR-027：文章列表顶部大图与缩略图（Slice 3；2026-09-25，用户选 A / 只放返回 / 保留紧凑栏小图标）

- 分三步、每步真机验收：① 缩略图（数据接入层从正文取首图 + Figma 规格）；② 顶部大图静态样子（展开 / 无图退路 / 紧凑栏）；③ 滚动跟手收缩（MOTION-CONTRACT §11）。
- 标题融入大图：**不照 Figma**（用户认为设计稿的虚化衬底块不好，授权自由设计）。采用 A「标题印在纸上」：订阅源高清图标（1.x `FeedHeroIconLoader`）放大虚化作氛围底，向下渐隐为纸色，标题用正文墨色落在已近纸色的底部；副标题「N 篇 · 刚刚更新」/同步中。否决 B 深色底白字（亮色图标需重压暗、与列表明暗断层）、C 居中刊头（长名换行不稳）。大图按 ADR-005 延伸到状态栏。
- 有大图时**不放**顶部圆形小图标（用户要求）；无高清图标时退回 Figma「No Hero Fallback」纸色大标题（同样不放图标）。
- 大图上**只放返回**：刷新 / 搜索 / 更多尚无功能，做出来再加，不放点了无反应的按钮。
- 完全收缩后的紧凑栏：保留「26pt 小图标 + 订阅源名」（Figma 03C）。
- 追加（2026-09-25 用户）：文章行有缩略图时，日期仍贴屏幕右边缘（第一行最右），缩略图在日期下方、与标题第一行顶端对齐；不照 Figma 把时间放在缩略图左侧。
- 追加（2026-09-25 用户“试试E”）：顶部大图由 A 改为 E「整幅清晰铺满」——订阅源图不虚化、完全不透明，只在底部约 45% 渐隐成纸色，标题落在渐隐区。否决 A（遮得太重，看不出图）；备选 D「左标题 + 右侧约 86pt 清晰方形封面 + 淡氛围底」（完整不裁、不重叠，冲击感弱），E 真机观感不理想时回退考虑。

## ADR-028：设置页按 Figma 一次做完（Slice 6；2026-09-25，用户“按照你的建议来”）

- 入口：首页左上角齿轮（此前首页无设置入口）。设置首页沿用路由恢复标识 babel2.settings。
- 结构：页面只依赖 `Babel2SettingsService` 接口；正式实现在接入层。旧设置存储（AppDefaults）只在 `Babel2LiveDataAdapters.swift` 读写，边界测试为该文件加 `AppDefaults.shared` 精确例外（与 AccountManager.shared 同一写法）。
- 交互：短选项用锚定弹出单选菜单（选完即生效并关闭，不跳页）；布尔用开关即时生效；API、发现服务、翻译模型为取消/保存编辑页；日志、统计、关于用现成旧页面以底部卡片弹出（保留其右上角按钮）；新增账户弹出现成登录页。
- 开关开启色用用户强调色（ADR-008），Figma 灰色视为默认中性色画法。
- 翻译模型页按规格重做：顶部刷新、热门 Top 10（带服务商 logo）、每服务商恰好 3 个（不足 3 个不列，最多 10 组）；当前选择不在列表中时单列于顶部。
- 不放（Babel 2.0 无对应功能，不放无效开关）：分组方式、刷新时清除已读、JavaScript、全屏文章、文章主题、添加 Babel 新闻源、配色模式子页（与“Select 行不跳页”冲突）、账户级新文章通知。
- 首页类别图标：7 个 Figma 生成占位图改用系统图标（统一次要灰），「翻译」用 Figma 正式矢量图标。
- 同时接通此前不生效或写死的设置：配色模式（整个 app）、文章排序、全部已读前确认、打开链接方式。

## ADR-029：文章列表搜索（Slice 3；2026-09-25，用户“按照你的建议来”）

- 原地搜索（交互合同 Feed Search）：放大镜在窄栏层 x=330；进入后大图收成窄栏、第二行换成搜索框 +「取消」、底栏隐藏；取消恢复原列表、档位、滚动位置。
- 范围：该订阅源**全部文章**，不受未读 / 星标 / 全部档位限制（找文章时通常不记得读没读过）。
- 空搜索框显示原列表（边打边筛）；停止输入 0.25 秒后搜索。
- 数据：数据库全文搜索（标题 + 正文）+ 标题 / 译文标题 / 摘要包含匹配两路合并（全文搜索按空格分词，中文需包含匹配补足；译文标题可搜中文）。
- 样式：Figma 无搜索界面稿，搜索框按设置页输入框近似（用户同意）。

## ADR-030：添加订阅页（Slice 6；2026-09-25，用户“按照你的建议来”）

- 新 Babel 2.0 页面替换「+」占位页；界面新写，搜索逻辑复用 1.x 发现引擎（`Shared/Discovery/`，只调用不改），账户操作只经接入层。
- 沿用 1.x 用户决定：单一搜索框自动判断输入、关键词四类并行分组（网站优先展开）、一类失败不连累其它、「订阅到」默认顶层不记住、点行试读、订阅后留在本页。
- 本次决定：搜索框在顶部（与列表搜索一致）；试读复用 1.x 页面以底部卡片弹出；取消订阅先确认；已订阅标记用中性灰（合同禁用旧绿色）；按键盘「搜索」才发起搜索（节省第三方额度）。
- Figma 无设计稿：样式按列表搜索框与设置行近似。

## ADR-031：文章列表大图的刷新与更多（Slice 3；2026-09-25，用户“按照你的建议来”）

- 位置照 Figma：刷新 x=201、搜索 x=330、更多 x=370，均在窄栏层全程不动。
- 刷新 = 刷新所有账户（同步按账户进行，无法只刷新单个源）。只在用户亲手点刷新时，同步结束后重载列表（保持滚动位置）；后台同步只显示状态、不重载，避免列表突然跳动。
- 更多菜单只放针对本订阅源的操作：打开网站主页、拷贝订阅地址、总是用阅读模式、新文章通知、重命名、取消订阅（先确认，只移除本账户的这个源）。不放「全部标为已读」（底栏已有）与「移到文件夹」（留给订阅管理页）。

## ADR-032：统一毛玻璃弹出菜单、大图按钮圆底、图标备份（2026-09-25，用户“两点都按你说的来”）

- 全 app 的弹出**菜单**统一为设置页选单那种毛玻璃（Babel2GlassMenu / Babel2GlassCard）：阅读页「•••」、大图「•••」、「全部标为已读」确认。屏幕中间的确认 / 输入**对话框**保留系统样式（键盘输入与读屏最可靠）。
- 大图上的四个顶部按钮加毛玻璃圆底并随收缩淡出；状态栏高度加淡纸色渐变。
- 订阅源小图标在接入层留备份，app 进后台不清（上游进后台清内存缓存的策略不改），只在内存警告时清。

## ADR-033：全 App 字号收小一档、底栏等距（2026-09-25，用户“都按你推荐的来”“开工吧”）

- 原字号来自 Figma 与 Reeder 截图，整体偏大；按对比预览统一收小约 12–15%，阅读正文 17pt / 行高 28。**本条取代 ADR-018 / 023 / 027 中的具体字号与图标尺寸**；版式结构不变。设置页不在范围内。
- 所有数字集中在 `iOS/Babel2/Babel2Type.swift`，以后调整只改这一处。
- 底栏五格改为 32 / 116.5 / 201 / 285.5 / 370（两端贴边、中间三格等距 84.5、左右对称），**取代** Figma 的 32 / 104 / 201 / 290.5 / 362。四条底栏共用 `Babel2BarLayout`。
- 动效（第二、三批）待本批验收后再做。

## ADR-034：全 App 动效统一规则（第二、三批；2026-09-26，用户“直接开始做第二批”“做完之后直接做第三批，我合并验收”）

- 只用三种时长：0.15 秒（按压）/ 0.22 秒（状态变化）/ 0.32 秒（页面级位移）；一种曲线 ease-out，不回弹；位移最多 8–12pt（「下一篇」例外：24 / 40pt）；只动位置、缩放、透明度。规则集中在 `iOS/Babel2/Babel2Motion.swift`。
- 系统「减弱动态效果」打开时：所有位移与缩放归零，只留淡入淡出（MOTION-CONTRACT §2 第 8 条）。跟手的动作（右滑返回、大图收缩、顶底栏显隐）不受影响。
- 首页与文章列表切档由 Figma 的 0.18 秒线性改为 0.22 秒 ease-out（合同 §4A 标注该值为 to-tune）。
- 加载中统一为呼吸占位条，取代「加载中…」文字；0.2 秒内完成的加载不显示占位条。
- 截图类过渡（切档、下一篇、阅读模式切换、搜索底栏）拍不到截图时一律放弃过渡、直接切换，不得把内容藏起来。
