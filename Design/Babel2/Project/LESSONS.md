...
## 42. 隐藏的视图照样参与自动布局；多余高度会被分给最「软」的标签（2026-09-25，文章行一行标题下沉）

- 现象：用户截图里一行标题的文章，标题比左侧图标低一截；两行标题的正常。量得每行都是 120pt，一行标题的标签被撑到 49pt、字被上下居中。原因是「行高 ≥ 缩略图底 + 14」在缩略图 isHidden 时仍然生效，多出的高度被分给竖直 hugging 最低的标题标签。
- 教训：isHidden 不会让约束失效；自适应行高里凡是「≥」约束都要问一句「这个元素不显示时它还在不在」。我的布局测试只用了两行标题的数据，没覆盖一行标题。
- 怎么应用：可选元素的约束随显示状态开关；文字标签竖直 hugging 设为 required，富余空白留在底部；布局测试至少覆盖「一行 / 两行 / 有无可选元素」三种组合，并断言标签高度等于文字所需高度。

## 43. 合同里的「shadow」要落实成投影，不是遮罩；无头测试碰不到转场，就自己造转场上下文（2026-09-25）

- 现象：用户报告设置页右滑返回时当前页整页变暗、闪一下。M1（2026-09-05）把合同的 `shadowOpacity = 0.18 × (1 − p)` 实现成盖在当前页上的整页黑色遮罩，手势一开始亮度就突降；所有页面的返回都是这样，只是设置页大片纯色最显眼。当时的测试因无头环境不触发转场（LESSONS 27）没覆盖到画面布置。
- 教训：视觉参数的命名（shadow / dim / scrim）决定画在谁身上、画成什么，实现前要对照系统通行效果想清楚；测试环境不触发的路径，要想办法直接喂输入去测，而不是默认「只能真机看」。
- 怎么应用：返回转场的布置由 `testPopDimsPreviousPageNotCurrentPage` 用假转场上下文守住；以后类似的转场（内置浏览器进入等）同样可以这样测布置。

## 44. 新加一种状态时，检查已有的滚动 / 手势规则在这种状态下还该不该生效（2026-09-25，搜索结果滑不动）

- 现象：用户报告搜索结果「下滑失灵」。模拟器探针显示触摸与内容都正常；真正原因是为大图收缩写的「松手停在半路就补完」在搜索状态下仍生效——搜索时没有大图，拖一小段松手就被拉回顶部。
- 教训：状态（搜索、编辑、弹窗）会改变前提；为另一种状态写的规则（补完、吸顶、隐藏栏）要逐条问「这里还适用吗」。排查时先用探针排除「触摸被拦截」「内容不够长」，再看手势回调。
- 怎么应用：列表页新增状态时，把 scrollViewDidScroll / WillEndDragging / 键盘处理逐一过一遍；测试直接调用手势回调断言结果。

## 45. 「先藏起来、等会儿再显示」的过渡，必须保证那个「等会儿」一定会来；按压缩放会改变 frame（2026-09-26）

- 现象：① 切档过渡先把列表设为透明、用旧列表截图盖住，数据到了再淡入；但系统拍不到截图（返回 nil）时，淡入那一步因为没有截图直接返回，列表永远透明。是新写的测试在无头环境碰巧拍不到截图时暴露的。② 按压反馈让按钮缩到 94%，而选中胶囊按按钮的 frame 定位，按下那一刻切档，胶囊就小一圈。
- 教训：任何「先改成中间状态、稍后恢复」的动画，恢复路径不能依赖一个可能不存在的前提；transform 会改变 frame（不改变 bounds 与 center），凡是读别人 frame 来定位的代码，在加缩放动画后都要复查。
- 怎么应用：截图类过渡一律「拍不到就不做」；定位用 center + bounds；测试里对「过渡中」的断言要允许两条路径，但「最终状态可见」必须无条件断言。
