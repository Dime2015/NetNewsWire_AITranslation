...
## T38 · 翻译按钮图标换代(系统 translate 符号)—— ✅ **已验收**(2026-07-30)

**六轮用户驱动的选型,过程记在 NNWTranslateIcon.swift 头注释,别推倒重来**:
单气泡「字」→ 自绘双气泡(嫌丑)→ **系统 SF「translate」符号(定案)**;
"已翻译"态:实心方块+镂空(被否)→ **图标不变 + 淡色圆角底衬**(tint 18%,深浅色自适应);
字号:17 显大 → 15 略大 → 14 太小 → **14.5 定案**(translate 是宽字形,同字号显大 ——
字号归 TranslationButton 自己管,控件板已声明不覆盖);底衬 26×21、角标 8pt/+8-7 随图标收紧。
