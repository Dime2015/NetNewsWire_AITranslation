...
## T9 · 模拟器里图标不跟随系统深浅色切换(判定为模拟器限制,待真机复核)

**状态**:判定为模拟器限制,**不是本项目的代码问题**;等装真机时顺手复核一次
**发现时间**:2026-07-21,用户提出"真机上图标会跟着主题实时变,这里为什么不变?"

### 现象

主屏图标外观设为「默认」时,把系统切到深色模式,
**NetNewsWire 的图标不变**,始终是浅色版。

### 排查过程与证据(结论建立在这四条上,不是推测)

1. **三种外观的素材确实都编进 app 了。**
   `xcrun assetutil --info <app>/Assets.car` 输出三条:
   `(默认)` / `UIAppearanceDark` / `ISAppearanceTintable`。
2. **手动指定时,深色/单色素材都能正确显示。**
   主屏长按 → 编辑 → 自定 → 选「深色」→ 立刻变成黑底米色 R;
   选「色调」→ 变成深底浅色 R。**说明素材和配置都没问题。**
3. **🔑 决定性证据:不跟随的不只是我们。**
   同一块屏幕上,Apple 自己的**「通讯录」和「文件」在深色模式下也是浅色版**。
   这两个系统 app 在 iOS 18+ 是有官方深色图标的。
   **别人家的 app 也不跟随 → 问题不可能出在我们的代码里。**
4. **排除了"只是没刷新"。**
   `xcrun simctl spawn booted launchctl kickstart -k system/com.apple.SpringBoard`
   重启 SpringBoard 后重新截图,图标依旧全是浅色 —— 而**日历小组件是深色的**,
   证明深色模式本身确实已生效。

### 结论

iPhone 17 / iOS 26.5 模拟器的 SpringBoard **不会按系统深浅色自动切换图标外观**,
只有在「自定」里手动指定时才切。这是模拟器的限制。

### 待办

装真机后(见 T6)**顺手切一次深浅色确认**。
按 L3 的纪律,这条在真机上验证过之前只算"高度可信的判断",不算已证实。
如果真机上也不跟随,再回来查我们的 Contents.json。
