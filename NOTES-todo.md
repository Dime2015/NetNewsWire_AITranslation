...
## T8 · app 图标:两个待用户决定的小问题

**状态**:待用户决定,都不影响使用
**发现时间**:2026-07-21,换图标后实测三种主屏外观 + 放大检查时发现
**更新**:2026-07-21 第二版素材(满铺无边框)已换上,两个问题都是针对第二版的

### 问题一 · 右下角的 wifi 图形被圆角切掉一块

满铺的图会被 iOS 的圆角遮罩裁掉四角,**遮罩半径约为边长的 22%**。
第二版素材把 wifi 图形放在右下角,最外圈那道弧正好落在裁切线上,被切断了。

正常尺寸下看不出来,把图标裁出来放大 6 倍才明显(见 L26 的验收方法)。

**修法**:让素材里的 wifi 图形往左上方挪一点,或者缩小一圈,离边缘留出余量。
需要用户重新出图。

### 问题二 · 「色调」主屏外观下,我们的图标和别人是反过来的

主屏长按 → 编辑 → 自定 → **色调**(iOS 把所有图标统一成一个色系):
别的 app 是**深蓝底 + 浅色图案**,我们是**浅底 + 蓝色 R** —— 明暗正好相反,
一排深色方块里夹一块亮的,不协调。

**原因**(已用 `xcrun assetutil --info Assets.car` 核实,不是推测):
编译产物里只有「默认」和「深色」两份图,**单色版是 iOS 自己生成的** ——
它把图转灰度,再按用户选的颜色上色。我们这张是**浅底**图,
灰度后大面积偏亮,上色后自然是"浅底深字"。

**这不是配置写错了,是素材明暗分布决定的。**
试过把 tinted 那条从 Contents.json 删掉让系统自己来 —— 结果一样,输入还是这张浅底图。

**修法**:做一张**深底浅字**的灰度版(例如黑底、白 R),放进
`AppIconCustom.appiconset`,并在 Contents.json 里加回 `luminosity: tinted` 那条。

### 为什么现在不修

「色调」是主屏的一个可选外观,**用户自己不一定会开**;
wifi 被切在正常尺寸下也基本看不见。两个都等用户看过截图再决定要不要出新素材。
