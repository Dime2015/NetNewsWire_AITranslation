...
enum TimelineStyle {
	...
	/// 标题飞到导航栏的**后半段**从哪里开始换成"系统那套标题"(只有首页用)。
	///
	/// 0.55 = 飞过一半之后才开始交接。前半段标题还大、还在头图里,那时它就是页面的门面;
	/// 只有快贴到导航栏时,才该变成导航栏该有的样子。
	static let headerDockedSwapStart = CGFloat(0.55)

	/// 深浅色切换时,头图交叉淡入的时长(秒)。
	///
	/// 为什么值得单独做这个动画:这套素材的浅色/深色多半是**同一场景的白天与夜晚**
	/// (太阳→月亮、THE NEW YORKER→PLAYBOY、点起烛火)。
	/// 硬切看起来是"图被换掉了",淡入才是"天黑了" —— 那正是这套素材的设计意图。
	/// 0.35 秒:短于系统深浅色切换的整体过渡感觉突兀,长于 0.5 秒又显得拖。
	static let headerAppearanceCrossfadeSeconds = 0.35
	...
	/// **订阅列表页(首页)专用**的头图高度比例。**故意比别的页矮一档。**
	///
	/// 理由(用户 2026-07-23 拍板):首页是拿来**找东西**的 ——
	/// 77 个源 + 7 个文件夹,让出 1/4 屏的代价比文章列表页大得多
	/// (文章列表本来就要往下滚,首页却是"一眼扫"的)。
	static let feedListHeaderHeightFraction = CGFloat(0.20)
	...
	// MARK: 垫在「头图 / 列表内容」之间的那层纸(2026-08-09)
	//
	// ## 它治的是什么 —— 来回三次才找准,前两次都只治了一条边
	//
	// 头图是 `collectionView.backgroundView`,**固定不动**,内容从它上面滚过去。
	// 而这一页的列表底色是**透明**的(`config.backgroundColor = .clear`,
	// 那是 2026-07-23 为了让头图能透出来才特意设的)——
	// 于是**卡片之间的缝、卡片左右的留白,露出来的全是头图**。
	//
	// 结果:每一块不透明的纸色元素都被头图**描了一圈边** ——
	// 分组标题栏是矩形(用户说的"直角")、分组卡片是圆角矩形("圆角")。
	//
	// 🔴 **前两版都错在"只想着最上面那一条边"**:
	// - v1:在内容上边缘画一条「纸色 → 透明」的带子。**方向反了**,
	//   反而在图上多画出一条新硬边(用户:「更丑了」)。
	// - v2:改成骑在交界线上的三段晕。最上面那条治好了,
	//   **但下面每一块卡片的边还在** —— 用户放大截图:「智能 Feed 和今天中间那个直角和圆角还在」。
	//
	// **正确的做法是垫一层纸,不是盖一块遮罩**:
	// 在**头图之上、列表内容之下**铺一层纸色底,上沿正好落在内容的上边缘(带一段羽化)。
	// 于是内容底下从头到尾都是纸,**卡片和缝隙同色 → 一条边都长不出来**;
	// 而内容上边缘以上仍然是图,一点没挡。
	//
	// 📌 判据:**边是"两种颜色相邻"才有的东西。与其一条条去糊,不如让相邻的两边同色。**
	// 遮罩是在"事后补救",垫底是"消掉产生边的前提"(L92 那条老规矩的又一例)。

	/// 纸色底板上沿的羽化高度(pt):在这段距离里从透明渐变到纸色,让头图化进纸里。
	/// **这是这块东西唯一的旋钮** —— 调大过渡更柔、但图的下半截被吃掉更多;调小更利落。
	static let headerContentBackdropFeather = CGFloat(44)
