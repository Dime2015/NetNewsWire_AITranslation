...
// MARK: - [翻译] 文章页宿主(试读 Phase C,2026-07-30)
//
// 原来这里是 ~200 行翻译/长图的 JS 桥接(本 fork 追加的)。试读页也要这两个功能,
// 桥接整体搬去了 Shared/Translation/NNWArticlePageHost.swift(协议扩展,两页共用),
// 本文件只剩这个 conformance —— webView 是 private,只有本文件够得着,这 6 行躲不掉。

extension WebViewController: NNWArticlePageHost {

	var nnwHostArticle: Article? { article }

	var nnwHostWebView: WKWebView? { webView }

	/// 标题译文在被阅读栏 CSS 藏掉的 DOM 里,用户看到的是 UIKit 画的那份 ——
	/// 把译文喂给阅读栏,标题才真正"被翻译"(2026-07-24 用户报的)。
