...
let pages: [Page] = [
	...
	Page(assetName: "HeaderArtFeedListAll",
		 ...
		 // ⚠️ **用户 2026-07-23 深夜换过图,坐标跟着重定过**(L72:素材换了坐标必须重量)。
		 // 新图里成捆的杂志**垒高成了三层的一面墙**(旧图只有一小堆),主体整体上移;
		 // 深色版的**月亮也更靠上**(约 3%~10% 高度处)。
		 // 所以纵向取到**几乎最顶**:1% 起,才不会把月亮和发髻切掉 ——
		 // 让掉的是底部那排杂志和榻榻米,反正它们本来就落在渐隐区。
		 crop: CGRect(x: 0.000, y: 0.010, width: 1.000, height: 0.772),
