...
**7. 🔴 不要自己去点模拟器验收 —— 界面上的事交给用户。**

> **这是本项目最贵的一条规则,已经被违反过一次并造成实际损失,读完再动手。**
>
> - 2026-07-21 用户第一次提出:别用 computer-use 点模拟器,**太慢太费 token**。
> - 2026-08-04 我用「本环境有专门的 iOS 模拟器 MCP 工具,又快又便宜」为由绕过了这条,
>   写进了 **L100**。
> - **2026-08-08 用户当场推翻:那个工具同样太费 token。**
>   截图是图片,每张都要进上下文 —— "专门的工具"并不改变成本,我当时那句"又快又便宜"是错的。
>
> **⚠️ L100 里"视觉改动必须自己截图看过"那半句作废。**
> L100 真正立得住的内核是**别盲交**(改了界面就不能靠猜),那一半继续有效 ——
> 只是**验收的执行者是用户,不是你**。
...
| 装进模拟器(`xcrun simctl install/launch`) | **你**,照旧(命令行,不费 token) |
| 看日志、查数据库、`curl` 验接口、跑纯逻辑自测 | **你**,照旧,而且**一个都不要省** |
| **在界面上点来点去、截图看效果** | **用户**。你说清楚点哪里、看什么、什么算对 |

**适用于一切"驱动界面"的工具**,包括但不限于:
`mcp__Claude_Code_iOS_Simulator__control`(screenshot / tap / swipe / touch_path…)、
computer-use、claude-in-chrome。**默认一律不用。**

所以每次交付的结尾应该是**一份让用户能照着做的验收清单**,例如:
> "装好了。请打开 设置 → 翻译模型 → 往下翻到 Anthropic 那一节,
>  应该看到 haiku / sonnet / opus 各一行(不是三行 opus)。不对就截图给我。"

而不是你自己去点。

**什么时候可以破例**:确实需要连续多步交互才能定位、且**别的手段都试过了**的疑难问题——
**而且必须先问用户、拿到同意**。问的时候要说清楚"我要点几步、为什么非点不可"。

**验收不了就明说。** 界面上的事你自己确认不了,交付时就写清楚
"这一条我没法自己验,请你看一眼" —— 不要把"应该没问题"写成"已验证"(L114/L119 的老账)。
...
>
> **这一条推翻了上面订阅发现那节里的「❌ 不做订阅管理(改名、删除、移动)—— 上游自己有」。**
> 当初那么写的理由是「上游自己有」。考古后确认:上游**只有单个操作**
> (主列表左滑 = 删除 / 重命名),而用户要的三件事上游**都没有**:
> **批量操作、拖拽整理、删文件夹时把里面的源释放到顶层**(上游是连里面的源一起删)。
> 所以这不是重复造轮子,是补上游的空白。
>
> 做什么:`+` 改为弹选单(文件夹管理 / 搜索订阅源),新增一个**文件夹管理页**,
> 支持新建 / 重命名 / 删除文件夹、移动源、批量删源。
> 同时**拿掉**账户分组头上那个「新建文件夹」按钮(功能并入管理页)。
>
> - ✅ 允许:新增管理页;**调用** Account 的公开接口(`addFolder` / `removeFolder` /
>   `renameFolder` / `moveFeed` / `removeFeed`)
> - ❌ **仍然不允许修改 A 级禁区的任何一行** —— 只调用,不改实现
> - ❌ 不做子文件夹:上游模型写死 `subfolders are not supported`(`Folder.folders` 恒为 nil),
>   层级永远只有「账户 → 文件夹 → 源」两层
>
> 用户已拍板的三个设计点(**别改回去**):
> 1. **多选「移动到…」为主,拖拽为辅** —— 用户有 77 个源,拖拽要边拖边滚屏、
>    还得先展开折叠的文件夹,批量场景下效率远低于多选
> 2. **删文件夹弹窗给两个选择**:释放源到顶层 / 连里面的源一起删
> 3. **批量删源接上游的撤销**(`Shared/Commands/DeleteCommand.swift` 本身就支持
>    批量 + UndoManager,白拿"摇一摇撤销")
>
> 分三阶段,**每阶段停下来等用户验收**:
> - **Phase A**:入口改造(`+` 选单、拿掉分组头按钮)+ 管理页展示 + 新建 / 重命名文件夹
> - **Phase B**:移动(多选「移动到」+ 拖拽)
> - **Phase C**:批量删除 + 删文件夹释放源
>
> 新代码放 `iOS/FolderManager/`(上游不存在此目录),改动带 `// [管理]` 标记。

> ⚠️ **第四次扩大范围(2026-07-23 晚,用户已确认):底部三档「星标 / 未读 / 全部」**
>
> 借鉴 Reeder:订阅列表页底部工具栏正中放一个**三档切换**,它是一个**全局档位** ——
> 决定列表里显示哪些源、每行显示什么数字、点进去看到哪些文章。
> 这是继订阅发现、文件夹管理之后**第三个功能性改动**(改的是"文章怎么筛",不只是外观)。
>
> 用户已拍板的四条(**别改回去**):
> 1. **拿掉订阅列表行上的左滑**(重命名/删除/更多)—— 左右滑要留给切档。
>    ✅ 查证过:**功能一个都不丢**,长按菜单里那一整套全都在(`makeFeedContextMenu` 等)
> 2. **「全部」档不显示任何数字**,就是一份最完整的源列表
> 3. **拿掉两处「漏斗」按钮**(订阅列表页右上角 + 文章列表页右上角)——
>    它们管的是同一件事,两个开关并存必然打架
> 4. **★ 档下智能组只留一行「全部星标」**(先做出来看效果),
>    未读/全部档下仍是「今天 / 全部未读」
>
> 分三阶段(**均已实现**,2026-07-23):
> - **Phase 1** ✅ 用户已验收:控件 + 未读/全部两档 + 拿掉行左滑 + 左右滑手势
> - **Phase 2** ✅ 待验收:★ 档打通(星标数 `NNWStarredIndex`、只留有星标的源、
>   点进去只看星标、智能组只留「全部星标」)
> - **Phase 3** ✅ 待验收:三个档各一张首页头图,切档交叉淡入
> - **追加** ✅ 待验收:**文章列表页底部也放这条控件**,原「搜索文章」输入框让位,
>   搜索改为导航栏右上角的放大镜按钮(`preferredSearchBarPlacement = .integratedButton`)。
>   ⚠️ **那一页不做左右滑手势** —— 它的行左滑是「加星标 / 标为已读」,有用,不能拿掉。
>
> 新代码放 `iOS/ReadingMode/`(上游不存在此目录),改动带 `// [阅读档]` 标记。
> 上游文件的改动通道:**要么"加一行调用",要么"一行换一行"**,实现全部住在我们自己的文件里。
>
> ⚠️ **左右滑切档的纯规则住在 `iOS/ReadingMode/ReadingModeRules.swift`**
>(不依赖 UIKit/Account),改完请跑 `tools/sim-readingmode.swift` ——
> 和拖放那套同一个理由:我点不了模拟器,能抽成纯逻辑的必须自己先验(L63)。
...
| 文件 | 唯一允许的改法 |
|---|---|
| `MainFeedCollectionViewController.swift` | **只允许**在 `add(_:)` 方法里加**一行**,调用本 fork 自己的扩展方法(右下角 `+` 的行为)。别的一律不碰 |

> 📌 **2026-07-23 更新**:这个文件曾经还有第二处改动 —— 分组头上装「新建文件夹」按钮的
> 4 行。做文件夹管理页时按用户要求**拿掉了那个按钮,那 4 行已还原成上游原样**
> (`iOS/MainFeed/AddFolderHeaderButton.swift` 一并删除)。
> 于是本文件的改动重新收敛回「`add(_:)` 里一行」。**别再往分组头上加东西。**
