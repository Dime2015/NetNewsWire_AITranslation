...
**为什么文章列表页本轮没动**:它和内容页是**两类不同的问题**,不该一起改。
内容页是"从来没有透明态"的真缺陷(已修);列表页**架构上已经是对的** ——
头图区时导航栏本来就透明、滚动后 scrim 毛玻璃渐显,两态都在,用户不满的是"程度不够"。
调"程度"要有参照物,等内容页的新效果出来、用户看到两页并排,才知道该往哪调
(一起改则两处互相干扰,出问题也不好定位)。
**下一轮的候选改法**:现在 scrim 的渐显是拿 `alpha` 从 0 推到 1 做的,而苹果明确说
**UIVisualEffectView 的 alpha 小于 1 时模糊会失真** —— 也就是渐变途中那层其实不是毛玻璃、
只是块半透明灰板。教科书做法是用 `UIViewPropertyAnimator` 驱动**毛玻璃浓度本身**
(⚠️ 该 animator 若在 active 状态被释放会崩,deinit 里必须 `stopAnimation`)。
