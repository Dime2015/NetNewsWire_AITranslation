...
**③ 滚动动画尚未做** —— 用户同意先做 ①②。做之前需要:
(a) 把标题从列表背景层挪到列表**上面**的浮层(否则会从文章文字底下穿过);
(b) 标题停靠时给导航栏淡入一层底(现在导航栏是透明的,否则字叠字);
(c) ✅ **用户已给出滚动后的截图(2026-07-22)**,证实了预判的问题:
    标题停靠在顶部居中时,**背后就是正在滚动的文章文字,没有底,字叠字**;
    而且停靠后用的是系统小标题(居中、无衬线、白色),与头图里的标题(靠右、衬线、主题色)
    完全是两个样子 —— 做"平滑飞过去"的动画之前,这两端必须先长得像。
另注:①(靠右)会让 ③ 的运动变成斜线,若嫌花哨可让标题停靠时仍靠右,或起点改回居中。
