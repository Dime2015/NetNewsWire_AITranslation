...
### 🚧 顶栏「渐变透明毛玻璃」(2026-07-23,**未达满意,交接后续**)

> ⚠️ **现状(2026-07-23 收尾):折腾多轮未达用户满意,当前落在「不出 bug 但不够透」**。
> 已试方案与坑的完整对照见 `NOTES-lessons.md` 末尾「顶栏『渐变透明』问题的现状小结」+ L59 / L60。
> 文章内容页当前 = 全 nil(系统默认毛玻璃,深浅正常但偏实);
> 文章列表页停靠 = ultraThin 毛玻璃。都不出 bug,但用户觉得不够透。**留待后续用更周全方案做。**
