...
**待用户验收**:①`+` 是否弹出两项选单;②进管理页能否看到 7 个文件夹和文件夹外的源;
③点文件夹整行能否展开看到里面的源;④右上角新建文件夹;⑤左滑重命名(文件夹和源都可)。
⚠️ **③ 是我最没把握的一项** —— 展开用的 `.header` 样式和 cell 点击有耦合,
我无法点模拟器验证,若点不开告诉我,改回「点小三角展开」即可。

**下一步**:Phase B(多选「移动到…」+ 拖拽),等本阶段验收通过再动。
