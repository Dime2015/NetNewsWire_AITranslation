...
//
//  ⚠️ **2026-08-08 起没有人调用本文件了。** 模型菜单重做之后(用户第 7 件)改成从
//  公开的 `/api/v1/models` 拉**全部**模型并按价格/厂商组织 —— 见 `OpenRouterCatalog.swift`。
//  本文件留着不删:它那套"内部接口随时会变"的防御式解析是一笔攒下来的经验,
//  将来若要再做一个「翻译榜」入口,捡回来即可。记档在 NOTES-todo T51。
//
//  ⚠️ 重要前提(实测得出,别想当然):
//
